...
for xpos in range(NumXTALPartitions_x):
    ...
    for ypos in range(NumXTALSubDomains):
        ...
        #-----------
        # GRAIN CALCUALTIONS.     # FOR LOCATIONS, ONLY THE Y-COORDINATES ARE NEEDED
        ...
        for SD_grain_count in range(NoGrains):
            # 1. Retrieve location of the datum plane where this sub-domain is to be sub-divided into grains
            DatPlnLoc = GrainBE_Locations
            #----BLOCK.START
            # CLAUSE - 1: This block only if the number of grains is more than 1
            # CLAUSE - 2: This block also does not execute at SD_grain_count==NoGrains; as there is no need for partitioning
            # 2. Build the name for the datum plane
            # 3. Make the datum plane
            # 4. Make the partition
            #----BLOCK.END
            # 5a. Build the grain name
            ThisGrainName = SDName + 'Grain_Nx_' + str(PartitionCount + 1) + '_Ny_' + str(GrainCount_x + 1)
            # 5b. Retrieve the centre location X-coord of this grain
            thisgraincentreX = thesesgrainshavecentreX
            # 5c. Retrieve the centre location Y-coord of this grain
            thisgraincentreY = GrainCentreLocations[SD_grain_count]
            # 5d. Test visually by printing out datum points at these location. KEEP COMMENTED
            #part.DatumPointByCoordinate(coords = (thisgraincentreX, thisgraincentreY, 0.0))
            # 6. Create set name
            # 7. Assign set
            # 8. Create section
            # 9. Create material name
            # 10. Assign the material
            # 11. Seed the top edges
            # 12. Seed the left edges
            # 13. Mesh the grain
        #-----------
...
#for SDnumber in ListOfSubDomains:
#    Get thickness of this sub domain

#    Calculate locations for partitioning datum planes NGrains [see SD_NGY_B, SD_NGY_A, SD_NGY_C to get values in NGrains]
#    Make datum planes where this sub-domain is to be partitioned
#    Make the partition
#    Give grain name - Use a GLOBAL grain name. No need to seperate.
#    Accumulate these grain names in an array
#    At the end of fully partitioning a sub-domain into grains, copy the array into a global dictionary with dub-domain name as key and grain names as values
#    Populate a new array having the centre locations of top edges
#    Populate a new array having the centre locations of left edges
