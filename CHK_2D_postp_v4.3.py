...
TimeInstances      = range(0, TotalNumFrames, frincr)
TimeSTEPValues     = range(0, TotalNumFrames, frincr)

ReactionForces_x   = np.zeros(len(range(0, TotalNumFrames, frincr)), dtype = 'float')
ReactionForces_y   = np.zeros(len(range(0, TotalNumFrames, frincr)), dtype = 'float')
...
for fr in range(0, TotalNumFrames, frincr):
...
TotalNumberOfGrains = int(NumPartitions_x * NumPartitions_y)
S11_ELEM_CENTROID   = dict.fromkeys(range(int(TotalNumberOfGrains)))
S22_ELEM_CENTROID   = dict.fromkeys(range(int(TotalNumberOfGrains)))
S12_ELEM_CENTROID   = dict.fromkeys(range(int(TotalNumberOfGrains)))
...
# key value 03: Number of elements per grain
# key value 04: maximum of the field variable
# key value 05: minimum of the field variable
# key value 06: mean of the field variable
# key value 07: standard deviation of the field variable
...
MAP_Grain_ElementsinGrain_S11 = dict.fromkeys(range(int(TotalNumberOfGrains)))
MAP_Grain_ElementsinGrain_S22 = dict.fromkeys(range(int(TotalNumberOfGrains)))
MAP_Grain_ElementsinGrain_S12 = dict.fromkeys(range(int(TotalNumberOfGrains)))
#-----------------------------------------------------------------
...
CalibStrings  = 'Loc' + Cal_Location + '__CalIter' + Calib_Iteration_Num + '__'
GSStrings     = str(NumPartitions_x*NumPartitions_y) + 'Ng_' + str(NumPartitions_x) + 'x_'+ str(NumPartitions_y) + 'y'
TEXStrings    = '_TEXID' + TEXIDVALUE
MeshStrings   = '_EBG' + EPGValue + '_' + ElementType
#    -    -    -    -    -    -    -    -    -    -    -    -    -
FileNamePart1 = CalibStrings + GSStrings + TEXStrings + MeshStrings
RandTerminatingNameIdentifier ='_id1_' + RandomNumber_START_MATLAB_OUTPUT + '_id2_' + RandomNumber_END_MATLAB_OUTPUT
FileNamePart3 = RandTerminatingNameIdentifier
#    -    -    -    -    -    -    -    -    -    -    -    -    -
# Res_Data_Root_Filename_1: time steps
Res_Data_Root_Filename_1 = FileNamePart1 + '____DATA_TSteps____' + FileNamePart3
# Res_Data_Root_Filename_2: reaction force x
Res_Data_Root_Filename_2 = FileNamePart1 + '____DATA_RFX____' + FileNamePart3
# Res_Data_Root_Filename_3: reaction force y
Res_Data_Root_Filename_3 = FileNamePart1 + '____DATA_RFY____' + FileNamePart3
# Res_Data_Root_Filename_4: UX_avg
Res_Data_Root_Filename_4 = FileNamePart1 + '____DATA_UXavg____' + FileNamePart3
# Res_Data_Root_Filename_5: UY_avg
Res_Data_Root_Filename_5 = FileNamePart1 + '____DATA_UYavg____' + FileNamePart3
# Res_Data_Root_Filename_6: Stress xx
Res_Data_Root_Filename_6 = FileNamePart1 + '____DATA_Stress_XX____' + FileNamePart3
# Res_Data_Root_Filename_7: Strain xx
Res_Data_Root_Filename_7 = FileNamePart1 + '____DATA_Strain_XX____' + FileNamePart3
# Res_Data_Root_Filename_8: Stress 11 - contour plot image file
Res_Data_Root_Filename_8 = FileNamePart1 + '____IMG_Stress_XX_CONTOUR____' + FileNamePart3 + '__RESFrameNum___'
# The next three names have already been defined mech before. So skip them.
...
# Res_Data_Root_Filename_12: Stress 22 - contour plot image file
Res_Data_Root_Filename_12 = FileNamePart1 + '____IMG_Stress_YY_CONTOUR____' + '_id1_' + FileNamePart3 + '__RESFrameNum___'
# Res_Data_Root_Filename_13: Stress 12 - contour plot image file
Res_Data_Root_Filename_13 = FileNamePart1 + '____IMG_Stress_XY_CONTOUR____' + '_id1_' + FileNamePart3 + '__RESFrameNum___'
#--------------------------------------------------------
# Write TimeStepData to file
outputFile = open(Res_Data_Root_Filename_1 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(TimeSTEPValues[fr]) + '\n')
outputFile.close()
# Write RF-X to file
outputFile = open(Res_Data_Root_Filename_2 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(RF_X_time_Data_SUM[fr]) + '\n')
outputFile.close()
# Write RF-Y to file
outputFile = open(Res_Data_Root_Filename_3 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(RF_Y_time_Data_SUM[fr]) + '\n')
outputFile.close()
# Res_Data_Root_Filename_4: UX_avg
outputFile = open(Res_Data_Root_Filename_4 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(U_X_time_Data_AVG[fr]) + '\n')
outputFile.close()
# Res_Data_Root_Filename_5: UY_avg
outputFile = open(Res_Data_Root_Filename_5 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(U_Y_time_Data_AVG[fr]) + '\n')
outputFile.close()
# Res_Data_Root_Filename_6: Stress xx
outputFile = open(Res_Data_Root_Filename_6 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(stress_xx[fr]) + '\n')
outputFile.close()
# Res_Data_Root_Filename_7: Strain xx
outputFile = open(Res_Data_Root_Filename_7 + '.txt', 'w')
for fr in range(0, TotalNumFrames, frincr):	outputFile.write(str(totalstrain_xx[fr]) + '\n')
outputFile.close()
#--------------------------------------------------------
# display and print S11 contours to image file
SESS_VP.odbDisplay.setPrimaryVariable(variableLabel = 'S', outputPosition = INTEGRATION_POINT, refinement = (COMPONENT, 'S11'), )
...
# display and print S22 contours to image file
SESS_VP.odbDisplay.setPrimaryVariable(variableLabel = 'S', outputPosition = INTEGRATION_POINT, refinement = (COMPONENT, 'S22'), )
# set contour display styles
session.viewports[VP_name].odbDisplay.contourOptions.setValues(numIntervals = 10) # 2 to 24
session.viewports[VP_name].odbDisplay.contourOptions.setValues(contourEdges = OFF)
session.viewports[VP_name].odbDisplay.contourOptions.setValues(contourEdgeThickness = VERY_THIN) # VERY_THIN, THIN, MEDIUM, THICK
...
session.viewports[VP_name].odbDisplay.commonOptions.setValues(visibleEdges = FREE) # only show free edges
session.viewports[VP_name].odbDisplay.commonOptions.setValues(edgeLineThickness = MEDIUM) # set model edge line thickness
# set max and min limits for S22 legend
SESS_VP.odbDisplay.contourOptions.setValues(maxAutoCompute = OFF, maxValue = S22_contour_label_max_MPa, minAutoCompute = OFF, minValue = S22_contour_label_min_MPa)
# set off title block
session.viewports[VP_name].viewportAnnotationOptions.setValues(title = OFF)
# loop over frames to display S22 contours
for fr in range(0, TotalNumFrames, frincr):
	SESS_VP.odbDisplay.setFrame(step = 'Step-1', frame = fr)
	SESS_VP.odbDisplay.setPrimaryVariable(variableLabel = 'S', outputPosition = INTEGRATION_POINT, refinement = (COMPONENT, 'S22'), )
	#SESS_VP.setValues(displayedObject = odb)
	if Print_S11_Contours_File == 1:
		if fr < 10:
			thisIMGfilename = Res_Data_Root_Filename_12 + '000'+ str(fr)
		elif fr >= 10 and fr < 100:
			thisIMGfilename = Res_Data_Root_Filename_12 + '00'+ str(fr)
		elif fr >= 100 and fr < 1000:
			thisIMGfilename = Res_Data_Root_Filename_12 + '0'+ str(fr)
		else:
			thisIMGfilename = Res_Data_Root_Filename_12 + str(fr)
		session.printOptions.setValues(reduceColors = True)
		session.printToFile(thisIMGfilename, PNG, (SESS_VP, ))
	else:
		dummy = 0
...
# display and print S12 contours to image file
SESS_VP.odbDisplay.setPrimaryVariable(variableLabel = 'S', outputPosition = INTEGRATION_POINT, refinement = (COMPONENT, 'S12'), )
# set contour display styles
session.viewports[VP_name].odbDisplay.contourOptions.setValues(numIntervals = 10) # 2 to 24
session.viewports[VP_name].odbDisplay.contourOptions.setValues(contourEdges = OFF)
session.viewports[VP_name].odbDisplay.contourOptions.setValues(contourEdgeThickness = VERY_THIN) # VERY_THIN, THIN, MEDIUM, THICK
...
session.viewports[VP_name].odbDisplay.commonOptions.setValues(visibleEdges = FREE) # only show free edges
session.viewports[VP_name].odbDisplay.commonOptions.setValues(edgeLineThickness = MEDIUM) # set model edge line thickness
# set max and min limits for S12 legend
SESS_VP.odbDisplay.contourOptions.setValues(maxAutoCompute = OFF, maxValue = S12_contour_label_max_MPa, minAutoCompute = OFF, minValue = S12_contour_label_min_MPa)
# set off title block
session.viewports[VP_name].viewportAnnotationOptions.setValues(title = OFF)
# loop over frames to display S12 contours
for fr in range(0, TotalNumFrames, frincr):
	SESS_VP.odbDisplay.setFrame(step = 'Step-1', frame = fr)
	SESS_VP.odbDisplay.setPrimaryVariable(variableLabel = 'S', outputPosition = INTEGRATION_POINT, refinement = (COMPONENT, 'S12'), )
	#SESS_VP.setValues(displayedObject = odb)
	if Print_S11_Contours_File == 1:
		if fr < 10:
			thisIMGfilename = Res_Data_Root_Filename_13 + '000'+ str(fr)
		elif fr >= 10 and fr < 100:
			thisIMGfilename = Res_Data_Root_Filename_13 + '00'+ str(fr)
		elif fr >= 100 and fr < 1000:
			thisIMGfilename = Res_Data_Root_Filename_13 + '0'+ str(fr)
		else:
			thisIMGfilename = Res_Data_Root_Filename_13 + str(fr)
		session.printOptions.setValues(reduceColors = True)
		session.printToFile(thisIMGfilename, PNG, (SESS_VP, ))
	else:
		dummy = 0
		del dummy
print 'Result_files_ID:'+RandomNumber_START_MATLAB_OUTPUT + '_id2_' + RandomNumber_END_MATLAB_OUTPUT
#--------------------------------------------------------
...
#volume = calculateCylinderVolume(10, 10)
#print volume
#-----------------------------------------------------------------
