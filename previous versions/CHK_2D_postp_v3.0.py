...
TIME_STEP_OF_INTEREST = 0 # this is to be later iterated upon
...
elmnum = 0
stress.getSubset(region=myInstance.elements[elmnum],position=CENTROID, elementType='CPS4').values

stress.getSubset(region=myInstance.elementSets[ElemSetName].elements[elmnum],position=CENTROID, elementType='CPS4').values
#print stress.getSubset(region=myInstance.elementSets[ElemSetName].elements[elmnum],position=CENTROID, elementType='CPS4').values[0]


#S11_ELEM_CENTROID = np.zeros((Num_DatumPlanes_y+1, Num_DatumPlanes_x+1))
# initialize a dictionary for the model for S11. Key holds grain number. Values include stress values
...
for jj in range(1, Num_DatumPlanes_y + 2):
	for ii in range(1, Num_DatumPlanes_x + 2):
		...
		thisgrainelements_S11 = range(0, len(Associate_ODB.rootAssembly.instances['PARTNAME-1'].elementSets[ElemSetName].elements), 1)
		thisgrainelements_S22 = range(0, len(Associate_ODB.rootAssembly.instances['PARTNAME-1'].elementSets[ElemSetName].elements), 1)
		thisgrainelements_S12 = range(0, len(Associate_ODB.rootAssembly.instances['PARTNAME-1'].elementSets[ElemSetName].elements), 1)
		for elem in range(0, len(Associate_ODB.rootAssembly.instances['PARTNAME-1'].elementSets[ElemSetName].elements), 1):
			# print Associate_ODB.rootAssembly.instances['PARTNAME-1'].elementSets[ElemSetName].elements[elem]
			# print stress.getSubset(region = myInstance.elementSets[ElemSetName].elements[elmnum],position = CENTROID, elementType = 'CPS4').values[0]
			# print stress.getSubset(region = myInstance.elementSets[ElemSetName].elements[elmnum],position = CENTROID, elementType = 'CPS4').values[0].elementLabel
			# extract S11 for this element at its centroid and store its value in key's values in dict S11_ELEM_CENTROID
			thisgrainelements_S11[elem] = stress.getSubset(region = myInstance.elementSets[ElemSetName].elements[elmnum],position = CENTROID, elementType = 'CPS4').values[0].data[0]
			thisgrainelements_S22[elem] = stress.getSubset(region = myInstance.elementSets[ElemSetName].elements[elmnum],position = CENTROID, elementType = 'CPS4').values[0].data[1]
			thisgrainelements_S12[elem] = stress.getSubset(region = myInstance.elementSets[ElemSetName].elements[elmnum],position = CENTROID, elementType = 'CPS4').values[0].data[3]
...
with open('myfile.txt', 'w') as f:
    print(S11_ELEM_CENTROID, file=f)
	
import cPickle as pickle

with open('file.txt', 'w') as file:
     file.write(pickle.dumps(S11_ELEM_CENTROID)) # use `pickle.loads` to do the reverse
#
